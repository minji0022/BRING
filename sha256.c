...
void sha256_final(SHA256_CTX *ctx, uint8_t *hash) {
    ...
    // Pad the message. Add a single '1' bit and zero bits until 56 bytes.
    ...
    for (i = 0; i < 8; ++i) {
        ctx->data[56 + i] = (ctx->bitlen >> ((7 - i) * 8)) & 0xFF;
    }
...
//     sha256_init(&ctx);
//     sha256_update(&ctx, (const uint8_t *)"Hello, SHA-256!", strlen("Hello, SHA-256!"));
